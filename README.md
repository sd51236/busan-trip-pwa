...
<head>
    ...
    <!-- PWA 設定 (使用相對路徑 ./ 以支援 GitHub Pages) -->
...
<body class="pb-24">
    ...
    <div id="ios-install-prompt" class="hidden fixed bottom-0 left-0 right-0 bg-white p-4 shadow-[0_-5px_20px_rgba(0,0,0,0.1)] z-[70] border-t border-gray-200 pb-8">
        <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
                <p class="text-sm font-bold text-gray-800">安裝「釜山旅遊手帳」</p>
                <p class="text-xs text-gray-500 mt-1">點擊下方工具列的分享按鈕 <i data-lucide="share" class="inline w-3 h-3 text-blue-500"></i>，然後選擇「加入主畫面」。</p>
            </div>
        ...
        <div class="flex justify-between items-start">
            <div>
                ...
                <div id="api-status" class="mt-1 flex items-center gap-1 text-[10px] text-gray-400 font-medium">
                    <span class="w-2 h-2 rounded-full bg-gray-300"></span> 離線模式
                </div>
            </div>
            <div class="bg-white border border-gray-100 p-2 rounded-2xl shadow-sm text-center min-w-[70px]">
                ...
                <div class="text-sm font-bold text-gray-700">18°C</div>
            </div>
        </div>
        <button onclick="app.resetData()" class="text-[10px] text-gray-400 underline mt-2 hover:text-red-500">重置資料</button>
    </header>

    <!-- 2. Checklist -->
    <div class="px-6 py-4">
        <h2 class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">行前清單</h2>
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t border-gray-200 px-6 py-3 z-50 flex justify-between items-center shadow-lg safe-area-pb">
        ...
        <button onclick="app.openModal('wallet-modal')" class="flex items-center gap-2 bg-orange-100 text-orange-800 px-4 py-2 rounded-full text-xs font-bold hover:bg-orange-200 transition-colors">
            ...
            <span id="footer-total" class="font-mono">₩0</span>
        </button>
    </div>
    ...
    <div id="add-modal" class="fixed inset-0 z-[60] hidden">
        ...
        <div class="absolute bottom-0 left-0 right-0 bg-white rounded-t-[2rem] p-6 modal-enter">
            <div class="flex justify-between items-center mb-6">
                <h3 class="text-xl font-bold font-serif text-gray-800">新增行程</h3>
            ...
            <form onsubmit="app.handleAddStop(event)" class="space-y-4">
                ...
                <input type="text" id="add-title" placeholder="標題" required class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-orange-500">
                <input type="text" id="add-location" placeholder="地點" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-orange-500">
    ...
    <div id="wallet-modal" class="fixed inset-0 z-[60] hidden">
        ...
        <div class="absolute bottom-0 left-0 right-0 bg-[#FDFBF9] rounded-t-[2.5rem] h-[85vh] flex flex-col modal-enter">
            ...
            <div class="px-8 pt-4 pb-6 flex items-center justify-between">
                <div><h2 class="text-2xl font-serif text-[#2C2C2C]">旅遊記帳</h2><p class="text-xs text-gray-400 mt-1 uppercase tracking-wider">自動同步</p></div>
            ...
            <div class="px-8 mb-6">
                <div class="bg-[#2C2C2C] text-[#F7F5F0] rounded-2xl p-6 shadow-xl">
                    <div class="flex justify-between items-start mb-4">
                        <div><p class="text-xs text-gray-400 uppercase tracking-widest mb-1">總花費</p><h3 class="text-3xl font-mono" id="wallet-total">₩0</h3></div>
                        <div class="text-right"><p class="text-xs text-gray-400 uppercase tracking-widest mb-1">人均</p><h3 class="text-xl font-mono text-orange-200" id="wallet-per-person">₩0</h3></div>
                    </div>
                    <div class="flex items-center gap-3 bg-white/10 p-2 rounded-xl w-fit">
                        ...
                        <span class="text-sm font-bold w-12 text-center" id="head-count-display">3 人</span>
                        <button onclick="app.changeHeadCount(1)" class="w-8 h-8 flex items-center justify-center bg-white/10 rounded-lg hover:bg-white/20">+</button>
                    </div>
                </div>
            </div>
            <div class="px-8 mb-6 flex gap-3">
                <div class="flex-1 flex flex-col gap-2">
                    <input type="text" id="exp-title" placeholder="項目" class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-orange-900">
                    <input type="number" id="exp-amount" placeholder="金額 (₩)" class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-orange-900">
                </div>
    ...
    <!-- JS Logic -->
    <script>
        // ⚠️ 如果您有部屬 Google Apps Script，請填入網址。若無則留空，App 會以「純離線模式」運作。
        const API_URL = ""; 
        ...
        const app = {
            ...
            // 🔥 小紅書熱門行程預設資料
            defaultItinerary: {
                day1: { id: 'day1', title: '第 1 天', subtitle: '南浦風情', stops: [
                    { id: 101, time: "10:00", title: "抵達飯店 & 放行李", desc: "建議住西面站，交通最方便。", tag: "交通", location: "西面站", mapLink: "https://map.naver.com/p/search/서면역", isImportant: false },
                    { id: 102, time: "11:30", title: "松亭三代豬肉湯飯", desc: "必吃！湯頭濃郁，記得加大量韭菜。", tag: "美食", location: "豬肉湯飯街", mapLink: "https://map.naver.com/p/search/서면돼지국밥골목", isImportant: false },
                    { id: 103, time: "14:00", title: "白淺灘文化村", desc: "必拍海岸隧道剪影，韓版聖托里尼。", tag: "景點", location: "白淺灘文化村", mapLink: "https://map.naver.com/p/search/흰여울문화마을", isImportant: true, isSpecial: true },
                    { id: 104, time: "18:00", title: "BIFF 廣場", desc: "晚餐吃路邊攤，必吃元祖黑糖餅。", tag: "美食", location: "BIFF廣場", mapLink: "https://map.naver.com/p/search/BIFF광장", isImportant: false }
                ]},
                day2: { id: 'day2', title: '第 2 天', subtitle: '海雲台', stops: [
                    { id: 201, time: "10:30", title: "天空膠囊列車", desc: "需預約！繽紛小火車配海景。", tag: "景點", location: "尾浦站", mapLink: "https://map.naver.com/p/search/해운대블루라인파크", isImportant: true, isSpecial: true },
                    { id: 202, time: "13:00", title: "青沙浦烤貝", desc: "邊看海邊吃現烤扇貝。", tag: "美食", location: "青沙浦", mapLink: "https://map.naver.com/p/search/청사포조개구이", isImportant: false },
                    { id: 203, time: "15:00", title: "灌籃高手平交道", desc: "拍列車經過瞬間。", tag: "景點", location: "青沙浦", mapLink: "https://map.naver.com/p/search/청사포", isImportant: false },
                    { id: 204, time: "19:00", title: "The Bay 101", desc: "摩天大樓夜景，拍倒影。", tag: "景點", location: "The Bay 101", mapLink: "https://map.naver.com/p/search/더베이101", isImportant: false }
                ]},
                day3: { id: 'day3', title: '第 3 天', subtitle: '廣安里', stops: [
                    { id: 301, time: "10:00", title: "Spa Land 汗蒸幕", desc: "五星級設施，必喝甜米釀。", tag: "彈性", location: "新世界百貨", mapLink: "https://map.naver.com/p/search/스파랜드", isImportant: false },
                    { id: 302, time: "16:00", title: "廣安里海景咖啡", desc: "找間咖啡廳躺著看海。", tag: "彈性", location: "廣安里", mapLink: "https://map.naver.com/p/search/광안리해수욕장", isImportant: false },
                    { id: 303, time: "20:00", title: "廣安大橋夜景", desc: "週六有無人機秀。", tag: "景點", location: "廣安大橋", mapLink: "https://map.naver.com/p/search/광안대교", isImportant: true }
                ]},
                day4: { id: 'day4', title: '第 4 天', subtitle: '返程', stops: [
                    { id: 401, time: "11:00", title: "田浦咖啡街", desc: "文創小店與選物店。", tag: "購物", location: "田浦站", mapLink: "https://map.naver.com/p/search/전포카페거리", isImportant: false },
                    { id: 402, time: "16:00", title: "前往機場", desc: "搭計程車去金海機場。", tag: "交通", location: "金海機場", mapLink: "https://map.naver.com/p/search/김해국제공항", isImportant: false }
                ]},
            },
            checklistItems: [{id: 'esim', label: 'eSIM'}, {id: 'passport', label: '護照'}, {id: 'wowpass', label: 'WOWPASS'}, {id: 'adapter', label: '轉接頭'}, {id: 'power', label: '行動電源'}],

            async init() {
                // Register Service Worker (使用相對路徑 ./ 以支援 GitHub Pages)
                ...
                if (API_URL && API_URL.includes("script.google.com")) {
                    document.getElementById('api-status').innerHTML = '<span class="w-2 h-2 rounded-full bg-orange-500 animate-pulse"></span> 同步中...';
            ...
            async fetchCloudData() {
                try {
                    ...
                    if (cloudData.success) {
                        ...
                        this.saveLocal();
                        this.renderAll();
                        document.getElementById('api-status').innerHTML = '<span class="w-2 h-2 rounded-full bg-green-500"></span> 雲端已同步';
                    }
                } catch (e) {
                    console.error(e);
                    document.getElementById('api-status').innerHTML = '<span class="w-2 h-2 rounded-full bg-red-500"></span> 同步失敗';
                }
            },
            ...
            renderItinerary() {
                ...
                if (stops.length === 0) { container.innerHTML = `<div class="text-center text-gray-300 text-sm py-10">點擊右下角 + 新增行程</div>`; return; }
                        ...
                        <div class="bg-white p-5 rounded-2xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.05)] border border-gray-50 relative">
                            ...
                            <div class="flex justify-between items-start mb-2 pr-6">
                                ...
                                <div class="flex gap-1">
                                    ${stop.isSpecial ? '<span class="text-[10px] px-2 py-0.5 rounded-full bg-pink-100 text-pink-500 font-bold">🔥 小紅書</span>' : ''}
                            ...
                            <div class="flex items-center gap-2 mt-3 pt-3 border-t border-gray-50">
                                <a href="${stop.mapLink || '#'}" target="_blank" class="flex items-center gap-1 text-xs font-bold text-gray-400 hover:text-blue-600 transition-colors" onclick="event.stopPropagation();"><i data-lucide="map-pin" class="w-3 h-3"></i> ${stop.location || '位置'}</a>
                            </div>
                        </div>
                    </div>`).join('');
            },
            ...
            async handleAddStop(e) {
                ...
                const newStop = { id: Date.now(), dayId: this.data.activeDay, time, title, location, tag, desc: "自訂行程", mapLink: `https://map.naver.com/p/search/${location}`, note: "", isImportant: false, isSpecial: false };
            ...
            updateTime() {
                ...
                document.getElementById('greeting').innerText = h < 12 ? '早安' : h < 18 ? '午安' : '晚安';
            }
        };
